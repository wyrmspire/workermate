...
| # | Task | Status | Notes |
|---|------|--------|-------|
| A.1 | Initialize Next.js project with TypeScript | ⬜ TODO | Run `npx create-next-app@latest . --typescript --app --src-dir --tailwind --eslint` from the repo root. Use App Router. Confirm `src/` directory structure is created. Do NOT overwrite existing `README.md` — decline or restore after scaffold. Verify `next dev` runs cleanly before proceeding. |
| A.2 | Install Genkit, Gemini, and Files API dependencies | ⬜ TODO | `npm install genkit @genkit-ai/google-genai @google/generative-ai zod`. These are the four core packages. `@google/generative-ai` is needed for `GoogleAIFileManager` (Files API upload). Do NOT install `@genkit-ai/firebase` or any other provider. Verify packages exist in `node_modules` after install. |
| A.3 | Create Genkit configuration module | ⬜ TODO | Create `src/agent/genkit.config.ts`. Import `genkit` from `genkit` and `googleGenAI` from `@genkit-ai/google-genai`. Initialize with `const ai = genkit({ plugins: [googleGenAI()] })`. Export the `ai` instance. The `GEMINI_API_KEY` env var is read automatically by the google-genai plugin. Do NOT hard-code any API key. Do NOT reference any model other than `gemini-3-flash-preview`. |
| A.4 | Define all Zod schemas in `src/lib/schemas.ts` | ⬜ TODO | Translate every interface from ARCHITECTURE.md Section 3 into Zod schemas. Must include: `OverlaySpecSchema` (with `boxes`, `lines`, `points`, `arrows` arrays — each element has x/y/w/h or x1/y1/x2/y2 as `z.number().min(0).max(1000)`), `ViewLayoutSchema`, `DimensionProposalSchema` (value: number, unit: enum mm/in, sourceViewId: string, confidence: number 0–1), `DatumProposalSchema`, `OrientationStepResultSchema` (proposalData: z.any(), overlay: OverlaySpecSchema, question: z.string(), optional cropWindow), `WizardStateSchema` (enum of IDLE, UPLOADED, ORIENT_STEP_1..5, ORIENT_LOCKED). Export all schemas AND their inferred types via `z.infer<>`. |
| A.5 | Define TypeScript types in `src/lib/types.ts` | ⬜ TODO | Create `OrientationSession` interface matching ARCHITECTURE.md exactly: `fileUri: string`, `currentState: WizardState`, `confirmedViews?: ViewLayout[]`, `confirmedEnvelope?: OverlaySpec`, `confirmedLW?: { length: DimensionProposal; width: DimensionProposal }`, `confirmedDepth?: DimensionProposal`, `lockedDatum?: DatumProposal`. Also define per-flow input types: `StepInput` base type with `fileUri: string`, `rejectionFeedback?: string`, plus step-specific extensions carrying prior confirmed data. Import Zod-inferred types from `schemas.ts` where possible to avoid duplication. |
| A.6 | Create mock fixtures for all 5 steps | ⬜ TODO | Create `src/lib/mocks.ts` with deterministic JSON fixtures matching every Zod schema. One `OrientationStepResult` per step with realistic overlay coordinates and questions. These fixtures power the mock mode so the entire UI can be developed and tested without calling Gemini. |
| A.7 | Add mock/live mode switch | ⬜ TODO | Add `NEXT_PUBLIC_MOCK_MODE=true` to `.env.example`. In a utility `src/lib/mode.ts`, export `isMockMode()`. API routes check this flag: if mock, return the fixture from `mocks.ts`; if live, call the Genkit flow. This lets the UI team iterate without burning API quota. |
| A.8 | Verify project compiles and dev server starts | ⬜ TODO | Run `npm run build` and `npm run dev`. Fix any TypeScript errors. Confirm the default Next.js page loads at `localhost:3000`. Confirm `src/agent/genkit.config.ts` compiles without errors. This is the gate — do not proceed to Lane B until this passes. |
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| C.1 | Implement `POST /api/upload` route | ⬜ TODO | Create `src/app/api/upload/route.ts`. Accept multipart form data with a single file field. Use `GoogleAIFileManager` from `@google/generative-ai` to upload the image to the Files API. Returns a `fileUri` (e.g., `https://generativelanguage.googleapis.com/v1beta/files/abc123`). Return JSON: `{ fileUri: string, fileName: string, mimeType: string }`. Validate file type is image (png, jpg, jpeg, tiff, pdf). Max file size: 20MB. Return 400 for invalid inputs. If `NEXT_PUBLIC_MOCK_MODE=true`, skip the actual upload and return a fake `fileUri`. |
| C.2 | Implement `POST /api/wizard/step-1` route | ⬜ TODO | Create `src/app/api/wizard/step-1/route.ts`. Parse JSON body: `{ fileUri: string, rejectionFeedback?: string }`. If mock mode, return the Step 1 fixture from `src/lib/mocks.ts`. Otherwise import `orientationFlowStep1_DetectViews` from `src/agent`. Call the flow with the parsed input. Return the `OrientationStepResult` JSON directly. Wrap in try/catch — on Genkit or Zod validation error, return 500 with `{ error: string, details: string }`. Set response headers: `Content-Type: application/json`. |
| C.3 | Implement `POST /api/wizard/step-2` route | ⬜ TODO | Create `src/app/api/wizard/step-2/route.ts`. Parse JSON body: `{ fileUri: string, confirmedViews: ViewLayout[], rejectionFeedback?: string }`. Validate `confirmedViews` is present and non-empty (return 400 otherwise). If mock, return fixture. Otherwise invoke `orientationFlowStep2_ConfirmEnvelope`. Return result. Same error handling pattern as C.2. |
| C.4 | Implement `POST /api/wizard/step-3` and `step-4` routes | ⬜ TODO | Create `src/app/api/wizard/step-3/route.ts` and `src/app/api/wizard/step-4/route.ts`. Step 3 body: `{ fileUri, confirmedViews, confirmedEnvelope, rejectionFeedback? }`. Step 4 body: `{ fileUri, confirmedViews, confirmedLW, rejectionFeedback? }`. Each validates required prior-step data is present. Mock or live depending on mode. Each invokes its corresponding Genkit flow and returns the result. Follow identical error handling pattern. |
| C.5 | Implement `POST /api/wizard/step-5` route | ⬜ TODO | Create `src/app/api/wizard/step-5/route.ts`. Body: `{ fileUri, confirmedViews, confirmedLW, confirmedDepth, rejectionFeedback? }`. If mock, return fixture. Otherwise invokes `orientationFlowStep5_FinalSummary`. On success, the returned `proposalData` contains `DatumProposal`. The frontend will use this to transition to `ORIENT_LOCKED`. Same error handling. |
